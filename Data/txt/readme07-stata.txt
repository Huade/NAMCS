  ...
  There are two ways to read the 2007 NAMCS and NHAMCS public-use data files into Stata:
  ...
  Option 1 - Use the self-extracting file NAMCS07-stata.exe, ED2007-stata.exe, or OPD07-stata.exe
  in the Stata folder on the FTP server to open a complete Stata dataset of the 2007 NAMCS, NHAMCS-ED, 
  or NHAMCS-OPD public-use files. 

  The steps for this option are as follows:

  1) Create a new folder on your local workstation, for example, C:\MYFILES\NAMCS07

  2) Download to the new folder the file NAMCS07-stata.exe from the FTP server: 
     ftp://ftp.cdc.gov/pub/Health_Statistics/NCHS/dataset_documentation/namcs/stata

  3) NAMCS07-stata.exe is a self-extracting compressed file which must be uncompressed prior to use.
     In order to do this, simply click on the file name in a Windows directory screen; a second
     file will appear in the same folder, about 33MB in size, with the name NAMCS07-stata.dta.  This is
     the Stata dataset.
  ...
  2) Download to the new folder the 2007 NAMCS dataset (NAMCS07.EXE) from the FTP server: 
     ftp://ftp.cdc.gov/pub/Health_Statistics/NCHS/datasets/namcs

  3) NAMCS07.EXE is a self-extracting compressed file which must be uncompressed prior to use.
     In order to do this, simply click on the file name in a Windows directory screen; a second
     file will appear in the same folder, about 33MB in size, with the name NAMCS07 and no file extension.
     Stata requires a file extension to recognize this as a data file, so simply rename it as NAMCS07.RAW

  4) Download to the new folder the NAMCS07.do and NAMCS07.dct files from the FTP server:
     ftp://ftp.cdc.gov/pub/Health_Statistics/NCHS/dataset_documentation/namcs/stata
