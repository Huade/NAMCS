   	...
   	for(i in 1:nPlotsTot){
   		...
   		if(any(subsetHold)){   	
   			...
   			pt1.1 <- xyplot(std.eff.sz ~ weighted | stopMeth, groups = whichVar, data = esDatTmp, 
   				scales = list(alternating = 1), ylim = c(-.05, 0.5), type = "l", col = ltBl, 
...
#	   	subsetHold <- plotTab$bigger & (as.factor(plotTab$stopMeth) %in% levels(as.factor(plotTab$stopMeth))[subset])
	   	...
	   	if(any(subsetHold)){
   			...
   			ylim = c(-.05, 0.5), type = "l", col = rdCol, par.settings = list(strip.background = list(col=stripBgCol)),
   			lwd = 2)
   			if(nullPlot){
   				ptHold <- pt1.2
   				nullPlot <- FALSE
...
#	   	subsetHold <- as.factor(plotTab$stopMeth) %in% levels(as.factor(plotTab$stopMeth))[subset]
   		...
   		pt2 <- xyplot(std.eff.sz ~ weighted | stopMeth, groups = sig >= .05, data = plotTab,
   			ylab = "Absolute standard difference", xlab = NULL, 
   			ylim = c(-.05, 0.5), type = "p", col = rdCol, pch = pchHold,
   			main = paste("Balance of ", ptNames[[i]][1], " versus ", ptNames[[i]][2], sep = ""),
   			#subset = subsetHold, 
    ...
    nullPlot <- TRUE
   	...
   	if(any(subsetHold)){
   		...
   		pt1.1 <- xyplot(effectSize ~ weighted | whichComp, groups = whichVar, data = esDatTmp, scales = list(alternating = 1),
   			ylim = c(-.05, 0.5), type = "l", col = ltBl, as.table = TRUE,
   	...
   	ylim = c(-.05, 0.5), type = "p", col = rdCol, pch = pchHold,par.settings = list(strip.background = list(col=stripBgCol)))
